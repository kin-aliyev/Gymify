...
fun WorkoutPlanDetailScreen(
    ...
    Box(
        ...
        if (state.isLoading) {
                    ...
                    WorkoutPlanBanner(
                        ...
                        onEditWorkoutClick = {
                            if (state.isWorkoutInDatabase) {
                                ...
                                onAction(WorkoutPlanDetailAction.OnEditPredefinedWorkoutClick(state.workoutPlanNameId))
                            }
                        },
                        ...
                        items(state.selectedExercises) { workoutExerciseWithExerise ->
                            AddedExerciseView(
                                ...
                                onEditClick = {
                                    if (state.isWorkoutInDatabase) {
                                        state.workoutId?.let {
                                            onAction( WorkoutPlanDetailAction.OnEditWorkoutClick(it) )
                                        }
                                    } else {
                                        onAction(WorkoutPlanDetailAction.OnEditPredefinedWorkoutClick(state.workoutPlanNameId))
                                    }
            ...
            // Кнопка Start с анимацией появления
            AnimatedVisibility(
                visible = state.isContentReady,
                enter = fadeIn(animationSpec = tween(400, easing = FastOutLinearInEasing)),
                modifier = Modifier
