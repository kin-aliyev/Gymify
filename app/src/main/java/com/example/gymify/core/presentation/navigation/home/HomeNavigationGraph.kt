...
import com.example.gymify.home.presentation.analytics_screen.AnalyticsState
    ...
    navigation<HomeNavigationGraph>(startDestination = Home) {
        composable<Home> {
            ...
            HomeScreen(
                ...
                state = state,
                onNavigateToAllUserWorkoutPlans = { navController.navigate(UserWorkoutPlans) }, //TODO создать экран всех планов
                onAction = { action ->
        ...
        composable<UserWorkoutPlans> {
            ...
            UserWorkoutsScreen(
                ...
                onAction = { action ->
                    when(action) {
                        is UserWorkoutsAction.NavigateBack -> { navController.popBackStack() }
                ...
                onAction = { action ->
                    ...
                    when (action) {
                        is WorkoutPlanDetailAction.OnDeleteWorkoutClick -> {
                            navController.navigate(Home) {
                                popUpTo(WorkoutPlanDetail(null)) { inclusive = true }
            ...
            WorkoutCompleteScreen(
                ...
                onAction = { action ->
                    ...
                    if (action is WorkoutCompleteAction.OnDoneClick) {
                        navController.navigate(Home) {
                            popUpTo(WorkoutComplete(0)) { inclusive = true }
                        }
                    }
                }
            )
